...
class TestReedSolomon : public CppUnit::TestFixture
{
	...
	CPPUNIT_TEST(test_Encode15_7);
	CPPUNIT_TEST_SUITE_END();
	
	public:
		void test_Encode15_7()
		{
			unsigned int GF = 4, N = 15, K = 7, phy = 0, len = 54;
			gr_top_block_sptr gt = gr_make_top_block("RSEncode15_7");
			...
			gr_char_to_float_sptr ch2f = gr_make_char_to_float();
			gt->connect(fi, 0, f2i, 0);
			gt->connect(f2i, 0, rs, 0);
			gt->connect(rs, 0, ch2f, 0);
			gt->connect(ch2f, 0, f2c0, 0);
