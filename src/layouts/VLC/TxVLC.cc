	...
	ly(_ly)
{
	...
	//PHY I
	int phr_mod = 0, psdu_mod = 0;
	if (vlc_var.phy_type==0) // PHY I
	{
		phr = PHY_I_modulator::Create(vlc_var.phy_type, vlc_var.mod_type, vlc_var._rs_code.pre_rs_in, vlc_var._rs_code.pre_rs_out, vlc_var.GF, vlc_var._cc_code.pre_cc_in,vlc_var._cc_code.pre_cc_out, vlc_var.PHR_raw_length,0)->self();
		phr_mod = boost::static_pointer_cast<PHY_I_modulator>(phr)->out_PHY_I_mod;
		psdu = PHY_I_modulator::Create(vlc_var.phy_type, vlc_var.mod_type, vlc_var._rs_code.rs_in, vlc_var._rs_code.rs_out, vlc_var.GF, vlc_var._cc_code.cc_in,vlc_var._cc_code.cc_out, vlc_var.PSDU_raw_length,vlc_var.operating_mode)->self();
		psdu_mod = boost::static_pointer_cast<PHY_I_modulator>(psdu)->out_PHY_I_mod;
	}
	else
	{
		phr = PHY_II_modulator::Create(vlc_var.phy_type, vlc_var.mod_type, vlc_var._rs_code.pre_rs_in, vlc_var._rs_code.pre_rs_out, vlc_var.GF, vlc_var.PHR_raw_length)->self();
		phr_mod = boost::static_pointer_cast<PHY_II_modulator>(phr)->out_PHY_II_mod;
		psdu = PHY_II_modulator::Create(vlc_var.phy_type, vlc_var.mod_type, vlc_var._rs_code.rs_in, vlc_var._rs_code.rs_out, vlc_var.GF, vlc_var.PSDU_raw_length)->self();
		psdu_mod = boost::static_pointer_cast<PHY_II_modulator>(psdu)->out_PHY_II_mod;
	}
	bbVLC_info_assembler::sptr INFO_ass = bbVLC_info_assembler::Create(vlc_var.mod_type, 3,vlc_var.psdu_units, phr_mod, psdu_mod);
	bbVLC_Frame_Generator::sptr FRAME_gen = bbVLC_Frame_Generator::Create(vlc_var.mod_type, vlc_var.flp_length,vlc_var.tx_mode, vlc_var.psdu_units,phr_mod, psdu_mod,3);
